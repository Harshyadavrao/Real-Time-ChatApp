...
function Signup(){
    ...
    return(
        <div className='w-full h-[100vh] bg-slate-200 flex justify-center items-center'>
            ...
            <form className='w-full flex flex-col gap-[20px] items-center justify-around mt-[20px]' onSubmit={handleSignup}> 
                <input type="text" name="" id="" placeholder='username' className='w-[90%] h-[50px] outline-none border-2 border-[#5ed45e] px-[20px] py-[10px] bg-white rounded-lg shadow-gray-300 shadow-lg' onChange={(e) => setUsername(e.target.value)} value={username}/>
                ...
                <button className='px-[20px] py-[10px] mt-[20px] bg-[#5ed45e] rounded-lg shadow-gray-300 shadow-lg text-[20px] font-semibold hover:shadow-inner'>sign up</button>
                <p className='cursor-pointer' onClick={() => navigate("/login")}>Already Have an Account? <span className='text-[#5ed45e] text-[bold]'>Login</span></p>
            </form>
            </div>
        </div>
    )
